...
async function newModuleProject(): Promise<void> {
  ...
  // Resolve install root for CMAKE_PREFIX_PATH on the build task.
  const installRoot = guessInstallRoot();
  ...
  await Promise.all([
    ...
    fs.writeFile(path.join(projectDir, '.vscode/tasks.json'),  TASKS_TPL(installRoot)),
...
/** Derive the install-tree root from `loom.runtimeExecutable`. The release
 *  tarball lays out bin/loom + lib/cmake/loom — we want the parent of bin/. */
function guessInstallRoot(): string {
  const { runtimeExecutable } = resolvePaths();
  if (!runtimeExecutable) return '${command:loom.installPathPlaceholder}';
...
# Pass -DCMAKE_PREFIX_PATH=<your-loom-install> to point CMake at the SDK.
# The .vscode/tasks.json in this folder does that automatically using the
# Loom install path that the LoomUI extension configured.
...
const TASKS_TPL = (installRoot: string) => `{
    "version": "2.0.0",
    "tasks": [
        {
            ...
            "args": [
                ...
                ${JSON.stringify(`-DCMAKE_PREFIX_PATH=${installRoot}`)}
            ],
            "problemMatcher": [],
            "group": "build"
        },
...
The \`.vscode/tasks.json\` in this folder is wired to the LoomUI extension's
configured Loom install path. Two ways to build:
...
If you're not using LoomUI, replace \`\${config:loom.moduleDir}\` in
\`tasks.json\` with the absolute path to your runtime's modules directory,
and pass \`-DCMAKE_PREFIX_PATH=<your-loom-install>\` to the configure step.
